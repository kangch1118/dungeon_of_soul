...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;try{
  ...
  const results=await page.evaluate(()=>{
    ...
    fresh();player=newPlayer();player.skills.E={id:'jobE',cd:0,cdMax:12,dmgMult:1.2,shots:5,radius:340};enemies=[enemy(2500,2500)];useSkill('E');check('E volley cannot cast out of range',player.skills.E.cd===0);
    enemies=[enemy(player.x+100,player.y)];const hp=enemies[0].hp;useSkill('E');for(let i=0;i<60;i++)updateTimedAttacks(1/60);check('E volley five shots on single target',Math.abs(hp-enemies[0].hp-player.atk*1.2*5)<.001);
    ...
    fresh();player=newPlayer();player.skills.E={id:'jobE',cd:0,cdMax:12,dmgMult:3.5,effect:'dash'};enemies=[enemy(player.x+90,player.y)];const dx=player.x,oldHp=enemies[0].hp;useSkill('E');for(let i=0;i<30;i++)updateTimedAttacks(1/60);check('E dash moves 180',Math.abs(player.x-dx-180)<.01);check('E dash hits once',Math.abs(oldHp-enemies[0].hp-player.atk*3.5)<.001);
