...
const SFX = {
  ...
  // 마법사 기본공격(마력탄)
  mage_basic: { src: "sound/magic_nomal.mp3", volume: 0.5 },
...
function bgmMasterVol() {
  return (
    (typeof options !== "undefined" && options.bgm != null ? options.bgm : 50) /
    100
  );
}
  ...
  optBgmSliderForBgm.addEventListener("input", () => {
    if (bgmAudio && bgmKey && BGM[bgmKey]) {
      bgmAudio.volume = Math.max(
        0,
        Math.min(1, BGM[bgmKey].volume * bgmMasterVol()),
      );
    }
  });
}
...
function playSfx(key) {
  ...
  const masterVol =
    (typeof options !== "undefined" && options.sfx != null ? options.sfx : 70) /
    100;
  if (masterVol <= 0) return;
  try {
    const priority = ["hurt", "monster_atk"].includes(key) ? 2 : 1;
    if (activeSfx.size >= 12) {
      const victim = [...activeSfx].find((v) => v.priority < priority);
      if (!victim) return;
      victim.audio.pause();
      activeSfx.delete(victim);
    }
    const audio = new Audio(def.src),
      voice = { audio, priority };
    activeSfx.add(voice);
    const release = () => activeSfx.delete(voice);
...
document.addEventListener("click", (e) => {
  ...
  if (
    e.target.closest(
      ".menu-btn, .img-btn, .corner-btn, .item-card, .shop-card, .equip-slot-box",
    )
  ) {
    playSfx("button");
  }
